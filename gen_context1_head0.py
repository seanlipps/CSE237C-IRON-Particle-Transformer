...
SOURCE_FILE = r"/notebooks/CSE237C-IRON-Particle-Transformer/iron_kernels/test_4_layer_1_context_head0.cc"
INCLUDE_DIR = r"/notebooks/CSE237C-IRON-Particle-Transformer/iron_kernels"

@iron.jit(is_placed=False)
def context1_head0_impl(input0, input1, output):
    N = input0.shape[0]
    N1 = input1.shape[0]
...
if __name__ == "__main__":
    # Load Inputs
    input0_data = iron.tensor(np.loadtxt(r"/notebooks/CSE237C-IRON-Particle-Transformer/temp_context1_head0_in0.txt", dtype=np.int8).flatten(), dtype=np.int8, device="npu")
    input1_data = iron.tensor(np.loadtxt(r"/notebooks/CSE237C-IRON-Particle-Transformer/temp_context1_head0_in1.txt", dtype=np.int8).flatten(), dtype=np.int8, device="npu")


    output_placeholder = iron.zeros((640,), dtype=np.int8, device="npu")
    
    context1_head0_impl(input0_data, input1_data, output_placeholder)

    np.savetxt(r"/notebooks/CSE237C-IRON-Particle-Transformer/temp_context1_head0_out.txt", np.array(output_placeholder, dtype=np.int8), fmt="%d")
