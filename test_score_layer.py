...
def scores(input0, output):
    N = input0.shape[0]  # Tensor size
    N_out = output.shape[0]
    element_type = output.dtype

    tap_in = TensorTiler2D.simple_tiler(input0.shape, tile_col_major=True)[0];
    ...
    of_x = ObjectFifo(in_ty)
    of_z = of_x.cons().forward()
    ...
    dense_ly_kernel = ExternalFunction(
        "scores",
        source_file=os.path.join(os.path.dirname(__file__), "iron_kernels/scores_kernel.cc"),
    ...
    rt = Runtime()
    with rt.sequence(in_ty, in_ty, out_ty) as (a_x, _, c_z):
        rt.start(worker)
        rt.fill(of_x.prod(), a_x, tap_in)
...
def main():
    ...
    Q_iron = iron.tensor(Q_np, dtype=np.int8, device="npu")
    S_iron = iron.zeros((Q_np.shape[1], Q_np.shape[0]), dtype=element_type, device="npu")

    print(Q_iron.shape);
    print(S_iron.shape)
    # Insantiate AIE Kernel
    scores(Q_iron, S_iron)
