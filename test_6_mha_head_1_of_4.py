...
def main():
    ...
    q_output = iron.zeros((INPUT_ROWS*ff_dim)//4, dtype=element_type, device="npu")
    k_output = iron.zeros((INPUT_ROWS*ff_dim)//4, dtype=element_type, device="npu")
    v_output = iron.zeros((INPUT_ROWS*ff_dim)//4, dtype=element_type, device="npu")

    score_output = iron.zeros(INPUT_ROWS*INPUT_ROWS, dtype=element_type, device="npu")
    context_output = iron.zeros(OUTPUT_SIZE, dtype=element_type, device="npu")
    ...
    out_np = np.array(context_output, dtype=np.int8)
