...
def layer_2_to_5(input0, input1, output, out_test):
    ...
    of_test = of_4.cons().forward(obj_type=out_ty, name="test")
    ...
    rt = Runtime()
    with rt.sequence(in_ty, in_ty, out_ty, out_ty) as (in_0, in_1, out_0, t_t):
        rt.start(*workers)
...
def main():
    ...
    inp1 = np.loadtxt("./data/a1_golden.txt", dtype=np.int8).flatten()
    ...
    layer_2_to_5(inp0_tensor, inp1_tensor, output, test)
